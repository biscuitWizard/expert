...
This document describes how `llm-gateway` integrates with llamacpp, runs the tool loop, routes feedback and domain actions, enforces guardrails, and handles summarization.

## 1. llamacpp integration

The service uses an HTTP client against llamacpp’s inference API:

- **Endpoints:** `/completion` or, when using OpenAI-compatible mode, `/v1/chat/completions`.
- **Streaming:** Server-Sent Events (SSE) for token or chunk streaming where supported.
- **Configuration:** Base URL, model parameters (for example `temperature`, `top_p`, `max_tokens`), and mode selection between native completion and chat-completions compatibility.
...
3. **Feed back** tool results to the model as follow-up message(s) in the chat sequence (or completion continuation, depending on API mode).
...
2. **Call** llamacpp with a dedicated **summarization system prompt** and the raw exchange text (or concatenated transcript).
